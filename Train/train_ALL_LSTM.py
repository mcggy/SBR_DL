...
def train(train_iter, dev_iter, test_iter, model, args):
    ...
    for epoch in range(1, args.epochs+1):
        ...
        for batch in train_iter:
            ...
            optimizer.zero_grad()
            logit = model(feature)
            loss = F.cross_entropy(logit, target, weight=torch.tensor([1, 100]).float().cuda())
            loss.backward()
...
def eval(data_iter, model, args, best_accuracy, epoch, test=False):
    ...
    for batch in data_iter:
        ...
        if args.cuda is True:
            feature, target = feature.cuda(), target.cuda()
        logit = model(feature)
        loss = F.cross_entropy(logit, target, weight=torch.tensor([1,125]).float().cuda(), size_average=False)
